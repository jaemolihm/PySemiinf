...
class TBdict:
    ...
    def get_hk(self, kvec):
        ...
        rdotk = np.einsum('xi,x->i', self.rvec, kvec)
        coeff = np.exp(1j * 2 * np.pi * rdotk) / self.ndegen
        hk = np.einsum('imn,i->mn', self.hr, coeff)
    ...
    def get_hk_fortran(self, kvec):
        """Fourier transform TBdict from real space to k space to get H(k)"""
        from get_hk import get_hk
        hk = get_hk(kvec, self.nw, self.nrpts, self.hr, self.rvec, self.ndegen)
        return hk
